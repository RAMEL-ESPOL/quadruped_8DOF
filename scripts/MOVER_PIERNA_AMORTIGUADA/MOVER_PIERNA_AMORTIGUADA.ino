...
BLDCMotor muslo = BLDCMotor(12,0.086f,380);
BLDCMotor pierna = BLDCMotor(12,0.086f,380);
...
Encoder m_sensor = Encoder(18, 19, 5000 );
//Encoder l_sensor = Encoder(18, 19, 5000 );
...
m_sensor.pullup = Pullup::USE_EXTERN;
  ...
  digitalWrite(M_OC_ADJ, HIGH); // for now put on high to give maximum over current limit 
 pinMode(M_FAULT_PIN, INPUT);

pinMode(L_FAULT_PIN, INPUT);
  ...
  digitalWrite(L_OC_ADJ, HIGH); // for now put on high to give maximum over current limit but better to connect it with a pot meter.
   if (digitalRead(L_FAULT_PIN) == LOW) {
  ...
  l_driver.voltage_limit = 23;
  pierna.current_limit = 15;
  ...
  muslo.monitor_variables = _MON_TARGET | _MON_VEL | _MON_ANGLE; 
  // downsampling
  muslo.monitor_downsample = 10; // default 10
  muslo.voltage_sensor_align = 1; // default 10
  ...
  pierna.voltage_sensor_align = 1; // default 10
  // initialize motor
   muslo.init();
